...
""".format(
            TEST_PRINCIPAL, TEST_PASSWORD, TEST_KEYTAB
        )

        expect = subprocess.Popen(
            ["expect"],
            shell=True,
...
def create_ccache():
    ...
    cmd = "kinit -S kadmin/admin -c {0} -kt {1} -p {2}".format(
        TEST_CCACHE, TEST_KEYTAB, TEST_PRINCIPAL
    )
...
class KAdminLocalUnitTests(unittest.TestCase):
    ...
    def test_each_iteration(self):
        ...
        self.logger.info(
            "each iteration {0} principals in {1} seconds. [{2} principals/second]".format(
                count[0], delta, (count[0] / delta)
            )
        )
...
def main():
    ...
    if True:
        ...
        kadmin_local_tests = unittest.TestLoader().loadTestsFromTestCase(
            KAdminLocalUnitTests
        )
